...
<body class="single inner-page">
                ...
                <div class="row">
                  ...
                  <div class="col-lg-12 form mb-3 ">
                    <div class="d-flex">
                      ...
                      <div class="col-lg-1">
                        ...
                        <input type="button" class="pay-button" id="goBtn" onclick="checkUserEmail(this)" value="Go" style="margin: 4px 5px;padding-left: 15px;">
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-lg-12 form mb-3 select-title <?= $blurclass ?>">
                    <div class="d-flex">
                      ...
                      <select name="user_initial_title" class="col-lg-1 <?= $disabledclass ?>" <?= $disabled ?> validate="Please select your title">
                        <option value="">Select Title</option>
                        <option value="Dr" <?php if ($userRec['user_title'] == 'DR') {
                                              echo 'selected';
                                            } ?>>Dr.</option>
                        <option value="Prof" <?php if ($userRec['user_title'] == 'PROF') {
                                                echo 'selected';
                                              } ?>>Prof.</option>
                        <option value="Mr" <?php if ($userRec['user_title'] == 'MR') {
                                              echo 'selected';
                                            } ?>>Mr.</option>
                        <option value="Ms" <?php if ($userRec['user_title'] == 'MS') {
                                              echo 'selected';
                                            } ?>>Ms.</option>
                      </select>
                              ...
                              <div class="acc-inclusion">
                                <div class="acc-icons">
                                  ...
                                  <?php if ($res_cal[0]['inclusion_sci_hall'] == "Y") { ?>
                                    <img src="<?= _BASE_URL_ . $cfg['EMAIL.HEADER.FOOTER.IMAGE'] . $result[0]['icon'] ?>" alt="" class="acc-icons-tooltip" title="Entry to Scientific Halls" />
                                  <?php } ?>
                                  <?php if ($res_cal[0]['inclusion_exb_area'] == "Y") { ?>
                                    <img src="<?= _BASE_URL_ . $cfg['EMAIL.HEADER.FOOTER.IMAGE'] . $result[1]['icon'] ?>" alt="" class="acc-icons-tooltip" title="Entry to Exhibition Area" />
                                  <?php } ?>
                                  <?php if ($res_cal[0]['inclusion_tea_coffee'] == "Y") { ?>
                                    <img src="<?= _BASE_URL_ . $cfg['EMAIL.HEADER.FOOTER.IMAGE'] . $result[3]['icon'] ?>" alt="" class="acc-icons-tooltip" title="Tea/Coffee during the Conference" />
                                  <?php } ?>
                                  <?php
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      if ($("input[type=radio][operationMode=registration_tariff]").length < 1) {
      ...
      function showSection(section) {
        ...
        if (section == 1) {
          ...
          if (storageEmail != '' && storageEmail !== undefined) {
            $.ajax({
              type: "POST",
              url: '<?= _BASE_URL_  ?>returnData.process.php',
              data: 'act=getEmailValidationStatusAbstract&email=' + storageEmail + '&altEmail=Y',
              dataType: 'json',

              success: function(response) {
                // console.log(response);

                if (response.STATUS == 'FACULTY') {
                  $('input[type=radio][operationmode=registration_tariff][value=5]')
                    .closest('label')
                    .removeClass('faculty-cat-enable');

                  $('#user_mobile').val(response.DATA.MOBILE_NO);
                  $("#goBtn").click();

                } else {
                  $('input[type=radio][operationmode=registration_tariff][value=5]')
                    .closest('label')
                    .addClass('faculty-cat-disable');
                }
              }
            });
          }
