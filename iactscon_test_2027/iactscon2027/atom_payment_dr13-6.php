<?php
	...
	//include_once("PG/WorldLineNew/TransactionResponseBean.php");
	...
	if(isset($_REQUEST['FROM']) && $_REQUEST['FROM']=='WL')
	{
			...
			if (isset($_POST['msg'])) 
			{
		        ...
		        $transactionResponseBean->key = $cfg['TEST.WL_NEW_ENC_KEY'];
		        $transactionResponseBean->iv = $cfg['TEST.WL_NEW_IV_KEY'];
		        ...
		        //echo '<pre>'; print_r($response_n); die;

		        $stat = $response_n[0];
		        $mystatus = explode("=", $stat);
	            $status = $mystatus[1];

	            $wamount = $response_n[6];
		        $mywamount = explode("=", $wamount);
	            $grandWAmnt = $mywamount[1];

	            $wbankCode = $response_n[4];
		        $bankCode = explode("=", $wbankCode);

		        $wclientCode = $response_n[3];
		        $clientCode = explode("=", $wclientCode);

		        $wdateTime = $response_n[8];
		        $dateTime = explode("=", $wdateTime);

		        $tpsl_txn_id = $response_n[5];
	            $w_tpsl_txn_id = explode("=", $tpsl_txn_id);
