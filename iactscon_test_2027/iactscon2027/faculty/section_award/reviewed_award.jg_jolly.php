...
?>
	<script language="javascript" src="scripts/award.jg_jolly.js"></script>
	<div class="container">
		<?php 
		switch($show){
...
<?php
	...
	?>
		<form name="frmSearch" id="frmSearch" action="reviewed_award.jg_jolly.process.php" method="post">
				...
				<tr>
					<td colspan="2" style="margin:0px; padding:0px;">
						...
						<table width="100%">
							...
							$searchCondition               = "";
							$searchCondition              .= " AND grandTAB.award_category = 'AWARD_DR_JG_JOLLY' 
							...
							?>
									<tr class="tlisting" <?=$rowStyleDecission?>>
										...
										{
										?>
											<a href="reviewed_award.jg_jolly.php?show=view&id=<?=$rowAwardDetails['id'].$searchString?>">Review</a>
										<?php
										}
										else if($rowAwardDetails['totalReviewAttemptCount']<2)
										{
										?>
											<a href="reviewed_award.jg_jolly.php?show=view&id=<?=$rowAwardDetails['id'].$searchString?>">Update Review</a>
										<?php
										}
										else
										{
										?>
											<a href="reviewed_award.jg_jolly.php?show=view&id=<?=$rowAwardDetails['id'].$searchString?>">View</a>
										<?php
										}
										?>
	...
	?>
		...
		<form name="frmAwardReview" id="frmAwardReview" action="reviewed_award.jg_jolly.process.php" method="post">
			...
			<table width="100%" class="tborder">
				...
				<tr>
					<td colspan="2" align="right">
						<a href="reviewed_award.jg_jolly.php?<?=$searchString?>">Back To Award Listing</a>
					</td>
				</tr>
				<tr>
							...
							<tr>
								<td align="left">
									...
									<input type="button" name="bttnWindowView" id="bttnWindowView" value="<?=($rowAwardDetails['totalReviewAttemptCount']<2)?'Cancel':'Back'?>" 
									 class="btn btn-medium btn-red" onclick="window.location.href='reviewed_award.jg_jolly.php?<?=$searchString?>'" style=" margin:10px 10px 10px 0;" />
