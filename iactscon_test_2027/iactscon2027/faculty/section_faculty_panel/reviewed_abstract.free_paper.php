...
<?php
							...
							if($resultAbstractDetails)
							{
								foreach($resultAbstractDetails as $i=>$rowAbstractDetails) 
								{
									...
									else
									{
										...
										if($rowAbstractDetails['totalReviewCount']>0)
										{
							?>
									<tr class="tlisting" <?=$rowStyleDecission?>>
										...
										{
										?>
											<a href="reviewed_abstract.free_paper.php?show=view&id=<?=$rowAbstractDetails['id'].$searchString?>">view</a>
										<?php
										}
	...
	?>
		...
		<form name="frmAbstractReview" id="frmAbstractReview" action="reviewed_abstract.free_paper.process.php" method="post">
