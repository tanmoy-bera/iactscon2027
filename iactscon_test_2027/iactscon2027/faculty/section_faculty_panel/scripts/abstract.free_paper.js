...
var reviewFrom = {
	...
	validation : function(){
		...
		$("input[operationMode=abstract_review_criteria_id]").each(function(){
			...
			if($("input[type=radio][operationMode=abstract_review_marks][criteriaId="+reviewCriteriaId+"]").length>0)
			{
				if($("input[type=radio][operationMode=abstract_review_marks][criteriaId="+reviewCriteriaId+"]:checked").length==0)
				{
