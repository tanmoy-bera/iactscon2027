...
switch ($act) {
    ...
    foreach ($brandingInputs as $inputName => $uploadType) {
        ...
        if (!empty($_FILES[$inputName]['name'])) {
            ...
            // Fetch old image
           if ($inputName == 'logo_image') {

				$sql = array();
				$sql['QUERY'] = "SELECT logo_image AS image FROM " . _DB_EMAIL_SETTING_ . " WHERE id=?";
				$sql['PARAM'][] = array('FILD'=>'id','DATA'=>$id,'TYP'=>'s');
          ...
          if ($inputName == 'logo_image') {
				$sqlUpdate = array();
				$sqlUpdate['QUERY'] = "UPDATE " . _DB_EMAIL_SETTING_ . "
									SET modified_dateTime=?
									WHERE id=?";
				$sqlUpdate['PARAM'][] = array('FILD'=>'modified_dateTime','DATA'=>date('Y-m-d H:i:s'),'TYP'=>'s');
				$sqlUpdate['PARAM'][] = array('FILD'=>'id','DATA'=>$id,'TYP'=>'s');
