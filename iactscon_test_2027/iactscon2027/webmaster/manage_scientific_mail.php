            ...
            <div class="page_top_wrap_left">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Manage Scientific Mail</li>
                    </ol>
                </nav>
                <h2>Manage Scientific Mail</h2>
                    ...
                    <table>
						...
						if ($result) {
							foreach ($result as $i => $row) {
								...
								$sidebar_image = _BASE_URL_ . $cfg['EMAIL.HEADER.FOOTER.IMAGE'] . $row['sc_sidebar_image'];

						?>
                            <tr>
                                <td>
                                    <img class="header_img" src="<?php echo $sidebar_image; ?>">
                                </td>
                               <td>
                                ...
                                <td class="action">
                                    <div class="action_div">
                                        <a data-id="<?=$row['id']?>" class="popup-btn icon_hover badge_secondary action-transparent br-5 w-auto editscisectionmailerbtn" data-tab="editscisectionmailer" ><?php edit(); ?></a>
                                    </div>
                                </td>
                            </tr>
                              <? } } ?>
  ...
  ///////////////////Section Edit///////////////////////////
   $(document).on('click', '.edimailsettinglistingBtn', function () {
    ...
    $.ajax({
       ...
       ///////////////////Section edit end///////////////////////////
         ///////////////////Section Edit///////////////////////////
   $(document).on('click', '.editscisectionmailerbtn', function () {
    ...
    $.ajax({
        url: 'includes/popup.php',
        type: 'POST',
        data: { editDocSciId: editDocId },
        success: function (response) {

            $('#editscisectionmailer').html($(response).find('#editscisectionmailer').html());

            // Re-initialize after DOM replacement
            initeditscisectionmailer();
               $('#editscisectionmailer').fadeIn();
