...
function editTagProcess($cfg, $mycms)
{
    ...
    $sqlTagListing			 = array();
    $sqlTagListing = array();
    $sqlTagListing['QUERY'] = "SELECT tags FROM " . _DB_USER_REGISTRATION_ . " WHERE `id` = ?";

    $sqlTagListing['PARAM'][]  = array('FILD' => 'id',  'DATA' => $userId,  'TYP' => 's');

    $resultTagListing   	 = $mycms->sql_select($sqlTagListing);
    $row = $resultTagListing[0];

    // 2. Handle NULL or empty
    if (empty($row['tags'])) {
        $existingTags = [];
    } else {
        $existingTags = array_map('trim', explode(',', $row['tags']));
    }
    ...
    if ($status == 1) {
        ...
        // REMOVE
        $existingTags = array_diff($existingTags, [$tag]);
    }
