...
{
?>
	<form name="frmSearch" method="post" action="accomodation_date.process.php" onSubmit="return FormValidator.validate(this);">
		...
		<table width="100%" class="tborder" align="center">
						...
						?>
							...
							?>
									...
									$sql['QUERY'] = "SELECT *								
																	  ...
																	  WHERE hotel_id = ?
																		AND status != ?";
									$sql['PARAM'][]	=	array('FILD' => 'hotel_id', 	'DATA' => $rowFetchHotelName['id'], 'TYP' => 's');
									...
									?>
										<td align="center" valign="top"><?= $seat_limit_text ?></td>
										<td align="center" valign="top"><?= $rowFetchcheckIn['checkin'] ?></td>
										<td align="center" valign="top"><?= $rowFetchcheckOut['checkout'] ?></td>
										<td align="center" valign="top">
											<a href="javascript:void(null);" onClick="redirectionOfLink(this)" ehref="add_accomodation_date.php?show=edit&id=<?= $rowFetchHotel['hotel_id'] ?>">
												<span alt="Edit" title="Edit Hotel" class="icon-pen"></span></a>
											<a href="accomodation_date.process.php?act=Remove&id=<?= $rowFetchHotel['hotel_id'] ?>">
			...
			<!-- <tr>
				<td colspan="5" align="right"><a href="add_accomodation_date.php?show=add">Add new</a></td>
			</tr> -->
			<tr class="tfooter">
				<td colspan="2">
...
{
?>
	<form name="frmInsert" id="frmInsert" action="accomodation_date.process.php" method="post" enctype="multipart/form-data" onsubmit="return onSubmitAction();">
		...
		<table width="100%" class="tborder">
				...
				<td colspan="2" style="margin:0px; padding:0px;">
					...
					<table width="100%">
						<tr class="thighlight">
							<td align="left">Hotel Name</td>
							<td>
								<select name="hotel_id" id="hotel_id" label="Section Name" validate="IsSel()" style="width:90%;" required="">
			...
			<tr>
				<td width="20%"></td>
				<td align="left">
					<input name="back" type="button" class="btn btn-small btn-red" id="back" value="Back" onClick="location.href='add_accomodation_date.php';" />
					&nbsp;
...
?>
	...
	<div>
		<button class="btn btn-success" id="date">Update all</button>&nbsp;&nbsp;
		<button class="btn btn-info" id="seat">Update Seat Limit</button>
		<a href="hotel_listing.php"><button class="btn" id="seat">Back</button></a>
	</div>
	<br>
	<div id="update_date" class='disableDiv'>
		<form name="frmUpdate" id="frmUpdate" action="accomodation_date.process.php" method="post" enctype="multipart/form-data" onsubmit="return updateConfirmation();">
			<input type="hidden" name="act" id="act" value="update" />
			<input type="hidden" name="id" id="id" value="<?= $rowFetchHotel['id'] ?>" />
			<table width="100%" class="tborder">
				<tr>
					<td class="tcat" colspan="2" align="left">Update Hotel Dates</td>
				</tr>
				<tr>
					<td colspan="2" style="margin:0px; padding:0px;">
	...
	<br>
	<div id="update_seat" class='disableDiv'>
		<!-- style="filter:blur(0.8px);pointer-events: none" -->
	...
	<script>
		...
		if (status == 'updateSeat') {
			$('#update_seat').removeClass('disableDiv');
			$('#update_date').addClass('disableDiv');
		}

		$(document).on('click', '#date', function() {
			$('#update_date').removeClass('disableDiv');
			$('#update_seat').addClass('disableDiv');
		})
