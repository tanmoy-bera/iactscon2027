<?php
	...
	switch($action)
	{
		...
		case'Remove':
			...
			$sqlUpdateHotel['QUERY']= "UPDATE "._DB_COMBO_CHECKIN_DATE_." 
									      SET `status` = ?
										WHERE `hotel_id` = ? ";
			...
			$sqlUpdateHotel	=	array();
			$sqlUpdateHotel['QUERY']= "UPDATE "._DB_COMBO_CHECKOUT_DATE_." 
									      SET `status` = ? 
										WHERE `hotel_id` = ?";
			$sqlUpdateHotel['PARAM'][]	=	array('FILD' => 'status' , 	  'DATA' => 'D' ,             'TYP' => 's');
			$sqlUpdateHotel['PARAM'][]	=	array('FILD' => 'hotel_id' ,  'DATA' => $hotel_id ,       'TYP' => 's');										   
			...
			while (strtotime($start_date) <= strtotime($end_date)) {
				...
				for($i = 0; $i < $count-1; $i++){
					...
					// INSERTING HOTEL DETAILS
						$sqlInsertHotel['QUERY']= "INSERT INTO "._DB_COMBO_CHECKIN_DATE_." 
						                                   SET `check_in_date` = ?, 
														       `hotel_id` = ?, 
															   `status` = ? ";
				...
				for($i = 1; $i < $count; $i++){
						$sqlInsertout	=	array();
					// INSERTING HOTEL DETAILS
						$sqlInsertout['QUERY']= "INSERT INTO "._DB_COMBO_CHECKOUT_DATE_." 
						                                   SET `check_out_date` = ? , 
														       `hotel_id` = ?, 
															   `status` = ? ";
